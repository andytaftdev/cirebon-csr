...
<div class="flex flex-col w-full min-h-screen">
    <div class="flex-grow p-7 mx-auto w-full max-w-screen-2xl h-full">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mt-10 space-y-4 md:space-y-0">
            <h2 class="text-2xl font-bold text-Ebony900 my-0">Kegiatan</h2>
            <a href="{{ route('kegiatan.create', $identity->id) }}"><button class="bg-AccentRed900 text-white text-sm flex justify-center items-center px-5 py-3 rounded-lg gap-2">+
    ...
    <a href="{{ route('kegiatan.filter', ['status' => '0']) }}">
                    ...
                    <tbody class="divide-y">
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                @if($item->status === 1)
                                <span class="px-2 p-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-Green50 text-Green600">
                                    Terbit
                                </span>
                                @else
                                <span class="px-2 p-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-Yellow100 text-AccentRed300">
                                    Draf
                                </span>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                                <div class="flex items-center gap-4">
                                    <a href="{{ route('kegiatan.detail', $item->id) }}" class="text-Ebony300 hover:text-Ebony900">
                                        <!-- Eye Icon -->
                                    ...
                                    <a href="{{ route('kegiatan.edit', $item->id) }}" class="text-Ebony300 hover:text-Ebony900">
                                        <!-- Pencil Icon -->
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
     ...
     <!-- pagination -->
            <div class="flex items-center justify-between bg-white px-6 py-3 mb-16 rounded-b-lg border-b border-l border-r border-gray-300">
