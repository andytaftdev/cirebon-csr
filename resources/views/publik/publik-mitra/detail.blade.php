...
<div class="w-full min-h-screen flex flex-col relative">
    <div class="w-full h-auto flex justify-center items-center relative">
        ...
        <div class="absolute w-3/4 mx-auto inset-0 flex flex-col items-start justify-center text-start z-20 p-5 lg:p-0">
            <p class="text-white text-sm md:text-base"><span class="text-OrangeS">Beranda / Mitra</span> / Detail</p>
            <h1 class="text-white text-3xl sm:text-4xl md:text-5xl lg:text-6xl max-w-screen-lg font-bold mt-3">{{$mitra->nama_mitra}}</h1>
            <div class="flex flex-col lg:flex-row mt-2 lg:mt-0 lg:gap-3">
                <p class="text-gray-300 text-sm md:text-base lg:my-4">PT {{$mitra->nama_mitra}}</p>
                <p class="text-gray-300 text-sm md:text-base lg:my-4 lg:block hidden font-bold">·</p>
                <p class="text-gray-300 text-sm md:text-base lg:my-4">{{$user->email}}</p>
                <p class="text-gray-300 text-sm md:text-base lg:my-4 lg:block hidden font-bold">·</p>
                <p class="text-gray-300 text-sm md:text-base lg:my-4">{{$mitra->nomor_hp}}</p>
            </div>
            <p class="text-gray-300 text-sm md:text-base">{{$mitra->alamat}}</p>
        </div>
    </div>

    <div class="px-7 md:px-0 md:w-2/4 mt-20 mx-auto">
        <div class="w-[40px] h-[4px] bg-Orange600"></div>
        <div class="w-full bg-gray-100 h-80 mt-8 flex justify-center items-center bg-cover bg-no-repeat" style="background-image: url('{{ asset('storage/img/profile/'. $mitra->mitra_logo) }}');"></div>
        <p class="mt-12 text-Ebony900">
            {{$mitra->deskripsi}}
        </p>

    </div>
    
    <div class="px-7 md:w-3/4 mx-auto mt-20">
        <div class="border-b border-Neutral300 mt-12 mb-6"></div>
        <div class="w-[40px] h-[4px] mt-20 bg-Orange600"></div>
        {{-- Laporan Dari Mitra --}}
        <h1 class="text-Ebony900 text-2xl md:text-3xl lg:text-4xl font-bold mt-4">Laporan Dari Mitra</h1>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-8 mt-7 p-2">
            <!-- Card 1 -->
